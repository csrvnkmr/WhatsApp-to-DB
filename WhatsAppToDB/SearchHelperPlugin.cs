...
namespace WhatsAppToDB
{
    public class SearchHelperPlugin
    {
        ...
        private OpenAiSettings aiSettings;

        public SearchHelperPlugin(IOptions<DatabaseSettings> dbSettings, IOptions<OpenAiSettings> aiSettings)
        {
            css = new VectorDBSync.ChromaSyncService(aiSettings.Value.ApiKey, dbSettings.Value.ChromaUrl, dbSettings.Value.ConnectionString);
            this.databaseSettings= dbSettings.Value;
            this.aiSettings=aiSettings.Value;
        }
        ...
        public async Task<string> GetCardName([Description("CardCode")] string cardcode)
        {
            ...
            try
            {
                using IDbConnection db = new SqlConnection(databaseSettings.ConnectionString);
