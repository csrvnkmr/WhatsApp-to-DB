...
namespace VectorDBSync
{
    internal class SQLiteVectorDBService : IVectorDBService
    {
        ...
        public SQLiteVectorDBService(IEmbeddingService embeddingService, string basePath = "Data")
        {
            ...
            List<Dictionary<string, object>>? metadatas)
        {
            ...
            for (int outer=0; outer < ids.Count; outer += batchsize)
            {
                if (outer % 1000 == 0)
                {
                    if (outer > 0)
                    {
                        var t1 = (LocalEmbeddingService)_embeddingService;
                        t1.Dispose();
                        GC.Collect();
                        GC.WaitForPendingFinalizers();
                        await Task.Delay(2000);
                    }
                    var tempEmbeddingService=new LocalEmbeddingService();
                    _embeddingService = tempEmbeddingService;
                }
                ...
                string dbPath = System.IO.Path.Combine(_basePath, $"{collectionName}.db");
        ...
        {
            string dbPath = System.IO.Path.Combine(_basePath, $"{collectionName}.db");
            if (!System.IO.File.Exists(dbPath)) return new List<SearchResult>();
